...
public class PressMenu extends BaseAbstractContainerMenu {
    ...
    public PressMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, (PressBlockEntity) inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2), new RestrictedItemStackHandler(slots));
    }

    public PressMenu(int pContainerId, Inventory inv, PressBlockEntity entity, ContainerData data, RestrictedItemStackHandler iItemHandler) {
        super(ModMenuTypes.PRESS_MENU.get(), pContainerId, slots);
