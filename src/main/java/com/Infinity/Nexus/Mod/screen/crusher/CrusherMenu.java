...
public class CrusherMenu extends BaseAbstractContainerMenu {
    ...
    public CrusherMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, (CrusherBlockEntity) inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2), new RestrictedItemStackHandler(slots));
    }

    public CrusherMenu(int pContainerId, Inventory inv, CrusherBlockEntity entity, ContainerData data, RestrictedItemStackHandler iItemHandler) {
        super(ModMenuTypes.CRUSHER_MENU.get(), pContainerId, slots);
