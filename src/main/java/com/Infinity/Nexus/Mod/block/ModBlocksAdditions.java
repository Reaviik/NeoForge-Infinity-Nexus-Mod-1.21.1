...
public class ModBlocksAdditions {
    ...
    public static final DeferredBlock<Block> COMPACTOR_AUTO = registerBlock("compactor_auto", () -> new CompactorAuto(BlockBehaviour.Properties.ofFullCopy(Blocks.IRON_BLOCK).requiresCorrectToolForDrops().strength(3.0f, 6.0f).lightLevel((state) -> state.getValue(Compactor.LIT) == 1 ? 2 : 0).noOcclusion().mapColor(MapColor.METAL)));
